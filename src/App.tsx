...
const WeatherWidget = () => (
  <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 flex items-center justify-between pointer-events-auto w-full h-full">
    <div className="flex flex-col">
      <span className="text-sm font-bold text-grafite-texto opacity-50 uppercase tracking-wider">São Paulo</span>
      <span className="text-4xl font-bold text-grafite-texto">24°C</span>
...
const NotepadWidget = () => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 flex flex-col pointer-events-auto w-full h-full">
      <div className="flex items-center gap-2 mb-3">
        <StickyNote className="w-4 h-4 text-brand-purple" />
        <h3 className="text-sm font-bold text-grafite-texto">Bloco de Notas</h3>
      </div>
      <textarea 
        value={note}
        onChange={(e) => setNote(e.target.value)}
        className="flex-1 bg-transparent border-none focus:ring-0 text-sm font-medium text-grafite-texto resize-none placeholder-gray-400"
    ...
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 flex flex-col pointer-events-auto w-full h-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-grafite-texto">Outubro</h3>
        </div>
        <span className="text-[10px] font-bold opacity-40 uppercase">2024</span>
      </div>
      <div className="grid grid-cols-7 gap-1 text-center">
        {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
          <span key={`${d}-${i}`} className="text-[9px] font-bold text-grafite-texto opacity-30">{d}</span>
        ))}
        {days.map(day => (
          <span 
            key={day} 
            className={`text-xs font-medium py-1.5 rounded-lg transition-colors ${day === today ? 'bg-brand-purple text-white shadow-md' : 'text-grafite-texto hover:bg-white/40'}`}
          >
            {day}
          </span>
        ))}
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 flex flex-col pointer-events-auto w-full h-full">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-bold text-grafite-texto">Comunicados</h3>
      </div>
      <div className="space-y-3">
        {announcements.map(a => (
          <div key={a.id} className="p-3 bg-white/40 rounded-xl border border-white/10 hover:border-brand-purple/20 transition-all cursor-pointer">
            <h4 className="text-xs font-bold text-grafite-texto">{a.title}</h4>
            <div className="flex justify-between items-center mt-2">
              <span className="text-[9px] text-on-surface-variant opacity-60 font-medium">{a.time}</span>
...
const Logo = () => (
  <div className="flex items-center gap-2 cursor-pointer group">
    ...
    <div className="flex items-baseline">
      <span className="text-xl font-bold text-grafite-texto tracking-tight">Komvos</span>
...
const TopBar = ({ 
  ...
  <header className="fixed top-0 left-0 right-0 bg-white/20 backdrop-blur-xl border-b border-white/10 z-50">
    <div className="flex justify-between items-center w-full px-8 py-2">
      ...
      <div className="flex items-center gap-4">
        ...
        <IconButton icon={<HelpCircle className="w-5 h-5 opacity-70" />} />
          ...
          <AnimatePresence>
            {isNotificationsOpen && (
              <motion.div
                ...
                className="absolute top-full mt-4 -right-4 w-80 sm:w-96 bg-white rounded-[32px] shadow-2xl border border-black/5 overflow-hidden z-[100]"
                style={{ transformOrigin: 'top right' }}
              >
                {/* Arrow */}
                <div className="absolute -top-2 right-6 w-4 h-4 bg-white rotate-45 border-t border-l border-black/5"></div>
...
const IconButton = ({ icon, onClick }: { icon: React.ReactNode, onClick?: () => void }) => (
  <button 
    onClick={onClick}
    className="p-2 text-grafite-texto hover:bg-white/10 rounded-full transition-all duration-200 active:scale-90 flex items-center justify-center"
  >
    {icon}
  </button>
);
...
const ClockWidget = () => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 flex flex-col items-center justify-center text-center pointer-events-auto w-full h-full">
      <span className="text-5xl font-bold text-grafite-texto tracking-tight">{formattedTime}</span>
      <span className="text-sm text-on-surface-variant mt-2 capitalize font-medium">{formattedDate}</span>
    </div>
  );
};
...
const TasksWidget = () => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 pointer-events-auto w-full h-full flex flex-col">
      <h3 className="text-sm font-bold text-grafite-texto mb-4">Tarefas</h3>
      <div className="flex-1 overflow-y-auto space-y-2">
        {tasks.map(task => (
          <div 
            key={task.id} 
            onClick={() => toggleTask(task.id)}
            className="flex items-center gap-3 p-2 hover:bg-white/40 rounded-xl cursor-pointer transition-colors group"
          >
            {task.completed ? (
            ...
            <span className={`text-sm ${task.completed ? 'text-on-surface-variant line-through opacity-50' : 'text-grafite-texto font-medium'}`}>
              {task.text}
            </span>
          </div>
        ))}
...
const ProjectsWidget = () => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg p-6 pointer-events-auto w-full h-full flex flex-col">
      <h3 className="text-sm font-bold text-grafite-texto mb-4">Projetos</h3>
      <div className="flex-1 overflow-y-auto space-y-3">
        {projects.map(p => (
          <div key={p.id} className="flex items-center justify-between p-2 hover:bg-white/40 rounded-lg transition-colors cursor-pointer group">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-grafite-texto">{p.title}</span>
            </div>
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50">
      <motion.div 
        ...
        className="bg-white/70 backdrop-blur-2xl rounded-xl px-2 py-1.5 flex items-center gap-1 shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-white/30"
      >
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute -top-12 whitespace-nowrap bg-white/90 backdrop-blur-md px-3 py-2 rounded-lg shadow-xl border border-cinza-claro/20 text-[11px] font-medium text-grafite-texto z-50 pointer-events-none"
          >
            {label}
          </motion.div>
        )}
...
const ContextMenu = ({ x, y, onClose, onOpenPersonalize }: { x: number, y: number, onClose: () => void, onOpenPersonalize: () => void }) => {
  return (
    <motion.div 
      ...
      className="fixed z-[100] bg-white/90 backdrop-blur-md rounded-xl border border-cinza-claro/20 shadow-xl py-2 w-56 flex flex-col pointer-events-auto"
      style={{ top: y, left: x }}
    >
...
const ContextMenuItem = ({ icon, label, onClick, disabled }: { icon?: React.ReactNode, label: string, onClick?: () => void, disabled?: boolean }) => (
  <button 
    disabled={disabled}
    onClick={onClick}
    className={`flex items-center gap-3 px-4 py-2 hover:bg-brand-purple/10 transition-colors text-sm font-medium ${disabled ? 'opacity-40 cursor-default' : 'text-grafite-texto'}`}
  >
    {icon}
    <span>{label}</span>
  </button>
...
const TabButton = ({ active, icon, label, onClick }: { active: boolean, icon: React.ReactNode, label: string, onClick: () => void, key?: React.Key }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-medium text-sm ${active ? 'bg-brand-purple text-white shadow-md shadow-brand-purple/20' : 'text-grafite-texto hover:bg-cinza-claro/10'}`}
  >
    {icon}
    <span>{label}</span>
  </button>
...
const AdminUsersView = () => {
  ...
  return (
    <div className="bg-white rounded-[32px] border border-black/5 overflow-hidden shadow-sm">
      <div className="p-6 border-b border-black/5 flex justify-between items-center bg-[#F9FAFB]/50">
        <h3 className="font-bold text-grafite-texto">Lista de Usuários</h3>
      ...
      <div className="divide-y divide-black/5">
        {users.map(u => (
          <div key={u.id} className="p-4 flex items-center justify-between hover:bg-black/5 transition-colors cursor-pointer group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-cinza-claro/20 rounded-xl flex items-center justify-center font-bold text-grafite-texto/50 text-xs">
                {u.name.split(' ').map(n => n[0]).join('')}
              </div>
              <div>
                <p className="text-sm font-bold text-grafite-texto">{u.name}</p>
                <p className="text-[10px] text-grafite-texto/50">{u.email}</p>
              </div>
            </div>
            <div className="flex items-center gap-8">
              <span className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest">{u.role}</span>
    ...
    return (
      <div className="flex flex-col items-center">
        <motion.div 
          ...
          animate={{ opacity: 1, scale: 1 }}
          className={`relative p-4 rounded-2xl border bg-white shadow-sm transition-all min-w-[200px] text-center cursor-pointer group ${isExpanded ? 'border-brand-purple ring-4 ring-brand-purple/5' : 'border-black/5 hover:border-brand-purple/50'}`}
          onClick={() => hasChildren && toggleNode(node.id)}
        >
          <div className={`w-10 h-10 mx-auto mb-2 rounded-xl flex items-center justify-center font-bold text-xs ${level === 0 ? 'bg-brand-purple text-white' : 'bg-cinza-claro/20 text-grafite-texto/50'}`}>
            {node.name[0]}
          </div>
          <h4 className="text-sm font-bold text-grafite-texto">{node.name}</h4>
          <p className="text-[10px] text-grafite-texto/50 font-bold uppercase tracking-wider">{node.role}</p>
          
          {hasChildren && (
            <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 p-1 bg-white border border-black/5 rounded-full shadow-sm group-hover:bg-brand-purple/10 group-hover:text-brand-purple transition-all">
...
const PersonalizationWindowContent = ({ 
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      {/* Sidebar Navigation */}
      <div className="w-64 border-r border-black/5 bg-[#F9FAFB] flex flex-col p-4 shrink-0">
        <div className="mb-8 px-2">
          <div className="flex items-center gap-2 mb-1">
            <Settings className="w-4 h-4 text-brand-purple" />
            <span className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest">Sistema</span>
          </div>
          <h2 className="text-sm font-bold text-grafite-texto">Configurações</h2>
        </div>
      ...
      <div className="flex-1 flex flex-col bg-[#F9FAFB] overflow-hidden">
        {activeTab === 'wallpaper' && (
          <div className="flex-1 p-10 overflow-y-auto space-y-8">
            <div>
              <h1 className="text-[32px] font-bold text-grafite-texto tracking-tight mb-2">Wallpaper</h1>
              <p className="text-sm text-grafite-texto/50 mb-8 font-medium">Escolha a atmosfera do seu espaço de trabalho.</p>
        ...
        {activeTab === 'widgets' && (
          <div className="flex-1 p-10 overflow-y-auto space-y-8">
            <div>
              <h1 className="text-[32px] font-bold text-grafite-texto tracking-tight mb-2">Widgets do Sistema</h1>
              <p className="text-sm text-grafite-texto/50 mb-8 font-medium">Configure quais módulos estarão visíveis na área de trabalho.</p>
              <div className="space-y-4 max-w-2xl">
                {widgets.map(w => (
                  <div key={w.id} className="flex items-center justify-between p-5 bg-white rounded-[24px] border border-black/5 shadow-sm group hover:border-brand-purple/20 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-[#F8F9FA] text-grafite-texto/40 rounded-2xl group-hover:bg-brand-purple/5 group-hover:text-brand-purple transition-colors">
                        <LayoutIcon className="w-5 h-5" />
                      </div>
                      <span className="font-bold text-grafite-texto">{w.name}</span>
                    </div>
                    <button 
                      onClick={() => onToggleWidget(w.id)}
                      className={`w-14 h-7 rounded-full transition-colors relative flex items-center px-1.5 ${w.visible ? 'bg-brand-purple' : 'bg-[#EEF0F2]'}`}
                    >
                      <motion.div 
        ...
        {activeTab.startsWith('admin_') && (
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="px-10 py-8 border-b border-black/5 bg-white">
              <h1 className="text-[32px] font-bold text-grafite-texto tracking-tight">
                {menuItems.find(i => i.id === activeTab)?.label}
              </h1>
              <p className="text-sm text-grafite-texto/50 mt-1 max-w-2xl leading-relaxed uppercase tracking-widest font-bold text-[10px]">
...
const WindowFrame = ({ 
  ...
  return (
    <motion.div 
      ...
      className={`fixed z-[100] flex flex-col bg-white/90 backdrop-blur-2xl border border-white/40 shadow-2xl overflow-hidden pointer-events-auto`}
      style={{ zIndex }}
    >
      {/* Window Header */}
      <div className="window-handle flex items-center justify-between px-4 py-2 border-b border-cinza-claro/5 cursor-grab active:cursor-grabbing bg-white/40 shrink-0">
        <div className="flex items-center gap-3">
          <span className="opacity-80 scale-90">{icon}</span>
          <h2 className="text-[11px] font-medium text-grafite-texto/90 tracking-tight">{title}</h2>
        </div>
        <div className="flex items-center -mr-2">
          <button 
            onClick={onMinimize} 
            className="p-2.5 hover:bg-black/5 transition-colors group"
            title="Minimizar"
          >
            <div className="w-3 h-[1px] bg-grafite-texto opacity-60 group-hover:opacity-100" />
          </button>
          <button 
          ...
          >
            <div className={`w-3 h-3 border border-grafite-texto opacity-60 group-hover:opacity-100 rounded-[1px] ${isMaximized ? 'relative' : ''}`}>
              {isMaximized && (
                <div className="absolute -top-1 -right-1 w-2 h-2 border border-grafite-texto bg-white/60" />
              )}
            </div>
          </button>
          <button 
    ...
    <div className="flex-1 flex flex-col bg-[#F9FAFB] overflow-hidden">
      ...
      <div className="px-10 py-8 border-b border-black/5 bg-white">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-[32px] font-bold text-grafite-texto tracking-tight">Preferências do usuário</h1>
            <p className="text-sm text-grafite-texto/50 mt-1 max-w-2xl leading-relaxed">
              Gerencie identidade, preferências de uso, disponibilidade de agenda e segurança da conta.
            </p>
          </div>
          <div className="flex gap-3">
            <button className="px-5 py-2 rounded-full border border-black/10 text-xs font-bold text-grafite-texto/70 hover:bg-black/5 transition-colors">
              Modo claro
            </button>
            <button className="px-5 py-2 rounded-full border border-black/10 text-xs font-bold text-grafite-texto/70 hover:bg-black/5 transition-colors">
              Sem time definido
      ...
      <div className="flex-1 p-10 overflow-y-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* Left Card: Perfil */}
          <div className="lg:col-span-5 bg-white rounded-[32px] border border-black/5 p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-grafite-texto">Perfil</h2>
              <span className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest">Imagem Pública</span>
            </div>
            <p className="text-sm text-grafite-texto/50 mb-8">Dados que aparecem para outros usuários.</p>
            
            <div className="bg-[#F8F9FA] rounded-3xl border border-black/5 p-12 flex flex-col items-center justify-center text-center mb-8">
              <div className="w-32 h-32 bg-[#EEF0F2] rounded-3xl flex items-center justify-center text-grafite-texto/40 text-4xl font-bold mb-6">
                VO
              </div>
              <h3 className="text-xl font-bold text-grafite-texto mb-1">Vinicius Otavio</h3>
              <p className="text-sm text-grafite-texto/50">vinicius@statum.com.br</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button className="bg-white border border-black/10 hover:bg-black/5 text-grafite-texto/50 py-3.5 rounded-xl font-bold text-sm transition-all">
                Remover
              </button>
            </div>
            <p className="text-[10px] text-center text-grafite-texto/30 mt-4">JPG, PNG, WEBP ou GIF com até 5 MB</p>
          </div>

          {/* Right Card: Dados e preferências */}
          <div className="lg:col-span-7 bg-white rounded-[32px] border border-black/5 p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-grafite-texto max-w-[200px] leading-tight">Dados e preferências</h2>
              <span className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest text-right">Aplicado ao entrar na plataforma</span>
            </div>
            <p className="text-sm text-grafite-texto/50 mb-8">Identidade operacional, tema e vínculo principal.</p>

            <div className="space-y-6">
              <div>
                <label className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest block mb-2 px-1">Nome de exibição</label>
                <input 
                  type="text" 
                  defaultValue="Vinicius Otavio"
                  className="w-full px-5 py-3.5 bg-white border border-black/10 rounded-2xl text-grafite-texto font-medium focus:ring-2 focus:ring-brand-purple/20 focus:border-brand-purple transition-all outline-none"
                />
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest block mb-2 px-1">Tema Padrão</label>
                  <select className="w-full px-5 py-3.5 bg-white border border-black/10 rounded-2xl text-grafite-texto font-medium focus:ring-2 focus:ring-brand-purple/20 focus:border-brand-purple transition-all outline-none appearance-none">
                    <option>Modo claro</option>
                    <option>Modo escuro</option>
                  </select>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest block mb-2 px-1">Time Principal</label>
                  <select className="w-full px-5 py-3.5 bg-white border border-black/10 rounded-2xl text-grafite-texto font-medium focus:ring-2 focus:ring-brand-purple/20 focus:border-brand-purple transition-all outline-none appearance-none">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest block mb-2 px-1">Dock — Visibilidade</label>
                  <select className="w-full px-5 py-3.5 bg-white border border-black/10 rounded-2xl text-grafite-texto font-medium focus:ring-2 focus:ring-brand-purple/20 focus:border-brand-purple transition-all outline-none appearance-none">
                ...
                <div>
                  <label className="text-[10px] font-bold text-grafite-texto/40 uppercase tracking-widest block mb-2 px-1">Dock — Posição</label>
                  <select className="w-full px-5 py-3.5 bg-white border border-black/10 rounded-2xl text-grafite-texto font-medium focus:ring-2 focus:ring-brand-purple/20 focus:border-brand-purple transition-all outline-none appearance-none">
              ...
              <div>
                <div className="inline-flex items-center px-4 py-2 bg-white border border-black/10 rounded-full text-[11px] font-bold text-grafite-texto/70 mt-4">
                  Time: <span className="text-grafite-texto ml-1">Sem time definido</span>
                </div>
              </div>
            </div>
          </div>
...
const DashboardCard = ({ name, detail, status, color, icon, progress, style = 'normal' }: any) => {
  if (style === 'classic') {
    return (
      <motion.div 
        ...
        className="p-6 bg-white/50 rounded-2xl border border-white/60 hover:bg-white/80 transition-all group shadow-sm hover:shadow-md h-full flex flex-col justify-between"
      >
        <div className="space-y-1">
          <h3 className="text-lg font-bold text-grafite-texto tracking-tight">{name}</h3>
          <p className="text-sm text-grafite-texto opacity-60 font-medium">{detail}</p>
        </div>
        <div className="mt-8 space-y-3">
          <div className="flex justify-between items-center hidden">
            <span className="text-xs font-bold text-grafite-texto/40">Progresso</span>
          </div>
  ...
  return (
    <motion.div 
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="p-6 bg-white/30 rounded-2xl border border-white/40 hover:bg-white/60 transition-all group shadow-sm hover:shadow-md flex flex-col h-full"
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-2xl font-bold text-grafite-texto tracking-tight">{status}</span>
      </div>
      
      <div className="mt-4">
        <h3 className="text-sm font-bold text-grafite-texto">{name}</h3>
        <p className="text-xs text-grafite-texto opacity-50 font-medium mb-4">{detail}</p>
...
const DashboardWindow = ({ onClose, onMinimize, zIndex, onFocus }: { 
  ...
  return (
    <WindowFrame 
      ...
      defaultHeight={450}
    >
      <div className="flex-1 p-10 overflow-y-auto bg-[#F8F9FA]/50">
...
const NotificationsWindowContent = () => {
  ...
  return (
    <div className="flex flex-col bg-[#F9FAFB] overflow-hidden max-h-[500px]">
      <div className="px-6 py-5 border-b border-black/5 bg-white">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl font-bold text-grafite-texto tracking-tight text-brand-purple">Notificações</h1>
            <p className="text-[10px] text-grafite-texto/50 mt-0.5 font-bold uppercase tracking-widest">Acompanhe as últimas atualizações</p>
          </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3">
        {notifications.map(n => (
          <div 
            key={n.id} 
            className={`p-4 rounded-2xl border border-black/5 shadow-sm transition-all flex items-start gap-3 ${n.read ? 'bg-white/50 opacity-60' : 'bg-white border-l-4 border-l-brand-purple'}`}
          >
            <div className={`p-2 rounded-lg shrink-0 ${n.read ? 'bg-black/5 text-grafite-texto/40' : 'bg-brand-purple/10 text-brand-purple'}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start mb-0.5">
                <h3 className="text-xs font-bold text-grafite-texto truncate pr-2">{n.title}</h3>
                <span className="text-[9px] font-medium text-grafite-texto/40 shrink-0">{n.time}</span>
              </div>
              <p className="text-[11px] text-grafite-texto/60 leading-relaxed line-clamp-2">{n.message}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="p-4 bg-white border-t border-black/5">
        <button className="w-full bg-[#F8F9FA] hover:bg-black/5 text-grafite-texto/50 py-2.5 rounded-xl font-bold text-[10px] transition-all uppercase tracking-widest">
...
export default function App() {
  ...
  const focusWindow = (type: 'personalize' | 'dashboard' | 'profile' | 'notifications') => {
    const nextZ = maxZ + 50;
    setMaxZ(nextZ);
  ...
  return (
    <div 
      className="min-h-screen flex flex-col pt-16 px-8 select-none relative transition-all duration-700" 
      style={{ 
